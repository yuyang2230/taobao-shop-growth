...
## 9. 商品视频绑定（选择视频弹窗）配方与边界（09-24 实战定案）

**流程**：编辑页点视频槽(emptyVideo 态) → 弹「选择视频」(sucai.wangpu.taobao.com 跨域 iframe) → 本地上传/选已有 → 点「确认上传(n)」→ 槽位落封面 → **立刻点「提交宝贝信息」**（绑定只存活于表单态，重载即丢——必须绑定后无缝提交）。

**已定案的能力边界**：
- **bsk snapshot 能穿透跨域 iframe**（选择器网格、确认上传按钮全部可见可读）→ 读状态/取证可用。
- bsk click 点不到卡片 hover 复选框（a11y 树不暴露、CSS 不穿域、无裸坐标点击）→ **选择动作 bsk 不可达**。
- CDP(playwright-core) 裸坐标能点中并绑定，但确认后 iframe 频繁 detach/重置、时序脆；**最可靠组合 = CDP 真实鼠标选中+确认后，同脚本内无缝提交**（一次成功全程复现：止回阀实测 PERSISTED）。
- 已绑定槽位再点击=打开视频预览播放器（非选择器）；替换走 video-play 菜单「替换/删除/视频裁剪」。
